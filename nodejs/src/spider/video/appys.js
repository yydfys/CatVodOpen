...
let host = 'http://app.zhaifutvapp.com/ruifenglb_api.php/v1.vod';
let header = {
...
async function home(inReq, outResp) {
    ...
    /*
    let promiseList = _.map(srcobj, (link) => {
        host = link[0].url;
        return request(host, getHeaders(host));;
    });*/
            ...
            if (jsonArray != null) {
                for (let i = 0; i < jsonArray.length; i++) {
                    ...
                    const filterStr = getFilterTypes(url, typeExtend);
                        ...
                        filterObj[Object.keys(srcobj)[i]] = filterArr;
...
/*
            ...
            // filterObj[Object.keys(srcobj)[i]] = JSON.parse(fjson);
        }
*/
...
async function category(inReq, _outResp) {
    ...
    try {
        ...
        let url = getCateFilterUrlPrefix(host, 1); // +  + getCateFilterUrlSuffix(apiUrl);
...
async function detail(inReq, _outResp) {
    ...
    for (const id of ids) {
        const apiUrl = host;
        const url = getPlayUrlPrefix(apiUrl) + id;
